...
def parse_cmdline_params():
    ...
    # ── new flag ──
    args.add_argument('--multiscale', action='store_true', default=False,
                      help='Use FPN + SpaceToDepth + LearnableWeightFusion head')
...
def log_scale_weights(net, writer, fold, epoch):
    """Logs learnable scale weights for the multiscale model."""
    # unwrap DataParallel
    module = net.module if hasattr(net, 'module') else net
    if not hasattr(module, 'get_scale_weights'):
        return
    w = module.get_scale_weights()   # (4,) on CPU
    labels = ['f1_fine', 'f2_mid', 'f3_high', 'f4_coarse']
    print(f"  [ScaleWeights] " + " | ".join(f"{l}:{v:.3f}" for l, v in zip(labels, w)))
    for label, value in zip(labels, w):
        writer.add_scalar(f'ScaleWeights/{fold}/{label}', value, epoch)
...
def main():
    ...
    class_mapping = {
        "grasp": 0, "retract": 1, "dissect": 2, "coagulate": 3, "clip": 4,
    ...
    for fold in range(args.kfold):
        print(f"\n{'='*60}")
        print(f"[INFO] Starting Fold {fold} | multiscale={args.multiscale}")
        print(f"{'='*60}")
        ...
        for epoch in range(start_epoch, args.nepochs):
            ...
            train_loss, train_map          = train(net, train_dataloader, loss_func,
                                                   optimizer, scheduler, scaler)
            valid_loss, valid_map, _, _, _ = valid(net, val_dataloader, loss_func)

            # ── log scale weights (multiscale only) ──
            if args.multiscale:
                log_scale_weights(net, writer, fold, epoch)
            ...
            state = {
                ...
                "epoch":             epoch,
                "multiscale":        args.multiscale,   # save flag for resume
            }

            if epoch % args.cpint == 0 and epoch != 0:
                torch.save(state, os.path.join(
                    args.cpdir, f"fold_{fold}_epoch_{epoch}.pt"
                ))

            if model_best:
                torch.save(state, os.path.join(
                    args.cpdir, f'fold_{fold}_model_best.pt'
                ))
            ...
            if early_stopping(valid_loss):
                print(f'[INFO] Early stopping at epoch {epoch} for fold {fold}.')
                break
